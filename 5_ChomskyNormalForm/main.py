...
class Grammar:
    ...
    def moveNonTerminals(self):
        for non_terminal in list(self.productions.keys()):  # Create a copy of keys for iteration
            productions = self.productions[non_terminal]
            production_id = 0
            while production_id < len(productions):
                production = productions[production_id]
                if len(production) == 1 and production != production.lower():
                    productions.pop(production_id)
                    productions.extend(self.productions.get(production, []))
                    # No increment of production_id here as we want to check the new production at this index
                else:
                    production_id += 1  # Only increment if no production was popped
